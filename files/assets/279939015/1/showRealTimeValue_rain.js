var ShowRealTimeValueRain = pc.createScript('showRealTimeValueRain');

// initialize code called once per entity
ShowRealTimeValueRain.prototype.initialize = function() {
...
ShowRealTimeValueRain.prototype.bindEventsAfterInit = function() {
    // ボタンのクリックイベントをバインドする
...
ShowRealTimeValueRain.prototype.bindButtonEvent = function() {
    const tryBind = () => {
...
ShowRealTimeValueRain.prototype.bindChartClick = function() {
    const tryBind = () => {
...
ShowRealTimeValueRain.prototype.handleClick = function(event) {
...
ShowRealTimeValueRain.prototype.updateButtonState = function(isActive) {
    // ボタンの視覚的状態を更新する
...
ShowRealTimeValueRain.prototype.startRealtimeUpdates = async function() {
    // 状態をリアルタイムモードに更新する
    if (this.chartCanvas) {
...
ShowRealTimeValueRain.prototype.fetchInitialData = async function() {
...
ShowRealTimeValueRain.prototype.setupRealtimeSubscription = function() {
    ...
    const tableName = AppConfig.SUPABASE3.TABLE_NAME;
    const selectedColumns = ['created_at', 'rain'];
...
ShowRealTimeValueRain.prototype.handleNewData = function(newData) {
    ...
    const yValue = parseFloat(newData.rain);
...
ShowRealTimeValueRain.prototype.showRealtimeZoomedChart = function() {
    const currentMode = this.chartCanvas.chartMode || 'none';
    if (currentMode !== 'realtime') {
...
ShowRealTimeValueRain.prototype.updateZoomedChart = function() {
...
ShowRealTimeValueRain.prototype.stopRealtimeUpdates = function() {
...
// ShowRealTimeValueRain.prototype.swap = function(old) { };

// learn more about scripting here:
// https://developer.playcanvas.com/user-manual/scripting/

// subFunctions
ShowRealTimeValueRain.prototype.initializeSupabase = function() {
...
ShowRealTimeValueRain.prototype.updateRealtimeChart = function() {
    if (this.realtimeData.length === 0) {
...
ShowRealTimeValueRain.prototype.drawRealtimeChartToCanvas = function(canvas, data, isZoomed = false) {
    ...
    ctx.fillText('雨量データ（リアルタイム）', canvas.width / 2, 30);
    
    // 統計情報を追加する
    const rainValues = chartData.rain.map(item => item.y);
    const rainValues_1h = chartData.rain_1h.map(item => item.y);
    const rainValues_24h = chartData.rain_24h.map(item => item.y);
    
    const rainStats = StatisticsUtils.calculateAllStats(rainValues);
    const rainStats_1h = StatisticsUtils.calculateAllStats(rainValues_1h);
    const rainStats_24h = StatisticsUtils.calculateAllStats(rainValues_24h);

    const statsText1 = `データ数: ${chartData.allDates.length} | 期間内平均: 雨量 ${rainStats.avg.toFixed(1)}mm`;
    const statsText2 = `期間内標準偏差: 雨量 ${rainStats.stdDev.toFixed(1)}mm`;
    ...
    const rangeRain = maxRain - minRain || 1;
    
    const minRain_1h = Math.min(...rainValues_1h);
    const maxRain_1h = Math.max(...rainValues_1h);
    const rangeRain_1h = maxRain_1h - minRain_1h || 1;
    
    const minRain_24h = Math.min(...rainValues_24h);
    const maxRain_24h = Math.max(...rainValues_24h);
    const rangeRain_24h = maxRain_24h - minRain_24h || 1;
    ...
    for (let i = 0; i <= yGridLines; i++) {
        ...
        ctx.textAlign = 'right';
        const rainValue = minRain + (i / yGridLines) * rangeRain;
        ctx.fillText(rainValue.toFixed(1), margin.left - (isZoomed ? 20 : 10), y + 4);
        
        // 左軸の目盛りラベル（1h雨量）
        ctx.fillStyle = '#2196F3';
        ctx.textAlign = 'left';
        const rainValue_1h = minRain_1h + (i / yGridLines) * rangeRain_1h;
        ctx.fillText(rainValue_1h.toFixed(1), margin.left + (isZoomed ? 20 : 10), y + 4);
    }
    ...
    for (let i = 0; i <= yGridLines; i++) {
        ...
        ctx.font = isZoomed ? '14px Arial' : '12px Arial';
        ctx.textAlign = 'left';
        const rainValues_24h = minRain_24h + (i / yGridLines) * rangeRain_24h;
        ctx.fillText(rainValues_24h.toFixed(1), rightAxisX + (isZoomed ? 20 : 10), y + 4);
    }
    ...
    for (let i = 0; i < chartData.rain.length; i++) {
        const timeRatio = (chartData.rain[i].x.getTime() - minTime) / rangeTime;
        const x = margin.left + timeRatio * width;
        const y = margin.top + height - ((chartData.rain[i].y - minRain) / rangeRain) * height;
    ...
    for (let i = 0; i < chartData.rain_1h.length; i++) {
        const timeRatio = (chartData.rain_1h[i].x.getTime() - minTime) / rangeTime;
        const x = margin.left + timeRatio * width;
        const y = margin.top + height - ((chartData.rain_1h[i].y - minRain_1h) / rangeRain_1h) * height;
    ...
    for (let i = 0; i < chartData.rain_24h.length; i++) {
        const timeRatio = (chartData.rain_24h[i].x.getTime() - minTime) / rangeTime;
        const x = margin.left + timeRatio * width;
        const y = margin.top + height - ((chartData.rain_24h[i].y - minRain_24h) / rangeRain_24h) * height;
    ...
    // 雨量点
    if (chartData.rain.length <= maxPointsToDraw) {
        ctx.fillStyle = '#FF5722';
        for (let i = 0; i < chartData.rain.length; i++) {
            const timeRatio = (chartData.rain[i].x.getTime() - minTime) / rangeTime;
            const x = margin.left + timeRatio * width;
    ...
    // 湿度ポイント
    if (chartData.rain_1h.length <= maxPointsToDraw) {
        ctx.fillStyle = '#2196F3';
        for (let i = 0; i < chartData.rain_1h.length; i++) {
            const timeRatio = (chartData.rain_1h[i].x.getTime() - minTime) / rangeTime;
            const x = margin.left + timeRatio * width;
    ...
    // 気圧ポイント
    if (chartData.rain_24h.length <= maxPointsToDraw) {
        ctx.fillStyle = '#4CAF50';
        for (let i = 0; i < chartData.rain_24h.length; i++) {
            const timeRatio = (chartData.rain_24h[i].x.getTime() - minTime) / rangeTime;
            const x = margin.left + timeRatio * width;
    ...
    */
    ...
    ctx.fillText('雨量 / 1時間累積雨量 (mm)', 0, 0);
    ctx.restore();
    ...
    ctx.rotate(-Math.PI / 2);
    ctx.fillText('24時間累積雨量 (mm)', 0, 0);
    ctx.restore();

    // 凡例を追加する
    const legendX = margin.left + width - (isZoomed ? 450 : 225);
    ...
    ctx.font = isZoomed ? '14px Arial' : '12px Arial';
    ctx.textAlign = 'left';
    ctx.fillText('雨量 (mm)', legendX + 25, legendY + 10);
    
    // 湿度図例
    ctx.fillStyle = '#2196F3';
    ctx.fillRect(legendX + 120, legendY, 20, 10);
    ctx.fillStyle = '#333';
    ctx.fillText('1H累積雨量 (mm)', legendX + 145, legendY + 10);
    
    // 気圧図例
    ctx.fillStyle = '#4CAF50';
    ctx.fillRect(legendX + 280, legendY, 20, 10);
    ctx.fillStyle = '#333';
    ctx.fillText('24H累積雨量 (mm)', legendX + 305, legendY + 10);
...
ShowRealTimeValueRain.prototype.formatRealtimeTime = function(date) {
    const now = new Date();
...
ShowRealTimeValueRain.prototype.onDestroy = function() {
    this.chartMode = 'none';
...
ShowRealTimeValueRain.prototype.convertToChartData = function(data) {
    // 元のデータをグラフ用の形式に変換する
    ...
    data.forEach(item => {
        ...
        if (dateObj && !isNaN(rain)) {
            ...
            rainData_1h.push({
                x: dateObj,
                y: parseFloat(getAccumulatedRain(data, dateObj, 60 * 60 * 1000).toFixed(2))
            });

            rainData_24h.push({
                x: dateObj,
                y: parseFloat(getAccumulatedRain(data, dateObj, 24 * 60 * 60 * 1000).toFixed(2))
            });
        }
    });
    ...
    return {
        rain: rainData,
        rain_1h: rainData_1h,
        rain_24h: rainData_24h,
        allDates: data.map(item => item[0])
    };
};


ShowRealTimeValueRain.prototype.formatTimeForAxis = function(date) {
...
ShowRealTimeValueRain.prototype.startSimpleHeartbeat = function() {
...
ShowRealTimeValueRain.prototype.sendSimpleHeartbeat = async function() {
    if (!this.isRealtimeActive || !this.supabaseClient) return;

    try {
        const { error } = await this.supabaseClient
        .from(AppConfig.SUPABASE3.TABLE_NAME)
        .select('created_at')
        .limit(1);
