var ShowCDFRain = pc.createScript('showCDFRain');

// initialize code called once per entity
ShowCDFRain.prototype.initialize = function() {
...
ShowCDFRain.prototype.bindClick = function() {
...
ShowCDFRain.prototype.handleClick = async function() {
    try {
        // 日付選択ダイアログを表示
...
ShowCDFRain.prototype.drawCDFChart = function(cdfData) {
    // グラフモードを更新する
    if (this.chartCanvas) {
...
ShowCDFRain.prototype.enableChartClickZoom = function() {
    const tryBind = () => {
...
ShowCDFRain.prototype.showCDFZoomedChart = function() {
...
ShowCDFRain.prototype.drawCDFZoomedChart = function(cdfData) {
    // 1. 全画面のオーバーレイを作成する
...
ShowCDFRain.prototype.calculateCDFData = function(data) {
    // 元のデータからCDF用データを計算する
    const rainValues = [];
    const rainValues_1h = [];
    const rainValues_24h = [];
    ...
    data.forEach(item => {
        ...
        const rain = item[1];
        const rain_1h = parseFloat(getAccumulatedRain(data, dateObj, 60 * 60 * 1000).toFixed(2))
        const rain_24h = parseFloat(getAccumulatedRain(data, dateObj, 24 * 60 * 60 * 1000).toFixed(2))
        
        if (!isNaN(rain)) rainValues.push(rain);
        if (!isNaN(rain_1h)) rainValues_1h.push(rain_1h);
        if (!isNaN(rain_24h)) rainValues_24h.push(rain_24h);
    });
    
    // CDFデータを準備
    const rainCDF = StatisticsUtils.prepareCDFForChart(rainValues, 100);
    const rainCDF_1h = StatisticsUtils.prepareCDFForChart(rainValues_1h, 100);
    const rainCDF_24h = StatisticsUtils.prepareCDFForChart(rainValues_24h, 100);
    
    // 統計情報を計算 - calculateAllStats は median を含まないため、calculateCDFSummary を使用
    const rainStats = StatisticsUtils.calculateCDFSummary(rainValues);
    const rainStats_1h = StatisticsUtils.calculateCDFSummary(rainValues_1h);
    const rainStats_24h = StatisticsUtils.calculateCDFSummary(rainValues_24h);
    
    return {
        rain: rainCDF,
        rain_1h: rainCDF_1h,
        rain_24h: rainCDF_24h,
        rainStats: rainStats,
        rainStats_1h: rainStats_1h,
        rainStats_24h: rainStats_24h,
        dataCount: data.length
    };
};


ShowCDFRain.prototype.drawCDFChartToCanvas = function(canvas, cdfData, isZoomed = false) {
    if (!cdfData) {
    ...
    ctx.fillText('雨量データ（確率分布）', canvas.width / 2, 30);
    
    // 統計情報を追加する - calculateCDFSummary を使用
    const statsText1 = `データ数: ${cdfData.dataCount} | 期間内平均: 雨量 ${cdfData.rainStats.mean.toFixed(1)}mm`;
    const statsText2 = `期間内標準偏差: 雨量 ${cdfData.rainStats.stdDev.toFixed(1)}mm`;
    ...
    // 各パラメータのCDFを描画
    this.drawCDFSubplot(ctx, 0, '雨量（mm）', cdfData.rain, cdfData.rainStats, 
                       margin, plotWidth, subplotHeight, isZoomed);
    
    this.drawCDFSubplot(ctx, 1, '1時間累積雨量（mm）', cdfData.rain_1h, cdfData.rainStats_1h,
                       margin, plotWidth, subplotHeight, isZoomed);
    
    this.drawCDFSubplot(ctx, 2, '24時間累積雨量（mm）', cdfData.rain_24h, cdfData.rainStats_24h,
                       margin, plotWidth, subplotHeight, isZoomed);
...
ShowCDFRain.prototype.drawVerticalLegend = function(ctx, canvas, margin, plotWidth, isZoomed) {
...
ShowCDFRain.prototype.drawCDFSubplot = function(ctx, index, title, cdfPoints, stats, 
    ...
    */
    ...
    let parameterLabel = '';
    if (index === 0) {
        parameterLabel = '雨量CDF';
    } else if (index === 1) {
        parameterLabel = '1時間累積雨量CDF';
    } else if (index === 2) {
        parameterLabel = '24時間累積雨量CDF';
    }
...
ShowCDFRain.prototype.getColorByIndex = function(index) {
    const colors = ['#FF5722', '#2196F3', '#4CAF50'];
    return colors[index % colors.length];
};

ShowCDFRain.prototype.drawCDFLegend = function(ctx, canvas, margin, plotWidth, subplotHeight, isZoomed) {
...
ShowCDFRain.prototype.fetchDataFromSupabase = async function(startDate, endDate) {
...
// イベントリスナーをクリア
ShowCDFRain.prototype.onDestroy = function() {
    if (this.entity.button) {
