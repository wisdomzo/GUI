var CameraManager2 = pc.createScript('cameraManager2');

// initialize code called once per entity
CameraManager2.prototype.initialize = function() {

    this.overviewCamera = this.app.root.findByName('OverviewCamera');
    this.targetCamera = this.app.root.findByName('Camera2');
    
    if (!this.overviewCamera || !this.targetCamera) {
    ...
    this.targetCameraOriginalPos = this.targetCamera.getLocalPosition().clone();
    this.targetCameraOriginalRot = this.targetCamera.getLocalEulerAngles().clone();
    this.targetCameraOriginalFov = this.targetCamera.camera ? this.targetCamera.camera.fov : 45;
    ...
    this.app.on('camera2:flydown:start', this.startFlyDown, this);
    this.app.on('camera2:reset', this.resetToOverview, this);
};


// 軌跡特徴を分析する
CameraManager2.prototype.analyzeTrajectory = function() {
    const start = this.overviewOriginalPos;
    const end = this.targetCameraOriginalPos;
    const startRot = this.overviewOriginalRot;
    const endRot = this.targetCameraOriginalRot;
...
CameraManager2.prototype.normalizeAngle = function(angle) {
...
// 最適な軌跡を自動選択する
CameraManager2.prototype.selectOptimalTrajectory = function() {
    // データに基づいて推奨する
...
// 軌跡タイプに応じて位置を計算する
CameraManager2.prototype.calculatePosition = function(t) {
    const start = this.overviewOriginalPos;
    const end = this.targetCameraOriginalPos;
...
// 放物線軌跡（急勾配ダイブ向け）
CameraManager2.prototype.calculateParabolaPosition = function(start, end, t) {
...
// ベジェ曲線（アーク移動向け）
CameraManager2.prototype.calculateBezierPosition = function(start, end, t) {
    // 制御点は中央で、やや上方向
...
// スプライン曲線（スムーズな回転向け）
CameraManager2.prototype.calculateSplinePosition = function(start, end, t) {
    // 線形補間
...
// 物理軌跡（総合的に考慮）
CameraManager2.prototype.calculatePhysicsPosition = function(start, end, t) {
    // イージングで速度を制御する
    let physicsT;
    if (t < 0.3) {
        // 加速を開始する
...
// 回転を計算する（大角度ジャンプを処理）
CameraManager2.prototype.calculateRotation = function(t) {
    const startRot = this.overviewOriginalRot;
...
// ダイブアニメーションを開始する
CameraManager2.prototype.startFlyDown = function() {
    if (this.isAnimating) {
        return;
    }
    ...
    const targetState = {
        progress: 1,
        fov: this.targetCameraOriginalFov,
        extraFov: 0
    };
...
CameraManager2.prototype.updateTweens = function(dt) {
...
// ダイブ完了
CameraManager2.prototype.onFlyDownComplete = function() {
    
    // 位置が一致していることを確認する
    this.overviewCamera.setLocalPosition(this.targetCameraOriginalPos);
    this.overviewCamera.setLocalEulerAngles(this.targetCameraOriginalRot);
    if (this.overviewCamera.camera) {
        this.overviewCamera.camera.fov = this.targetCameraOriginalFov;
    }
    ...
    this.app.fire('camera2:flydown:complete');
...
// リセット
CameraManager2.prototype.resetToOverview = function() {
    // 重複処理を防ぐ
...
CameraManager2.prototype.destroy = function() {
    this.isAnimating = false;
    ...
    this.app.off('update', this.updateTweens, this);
    this.app.off('camera2:flydown:start', this.startFlyDown, this);
    this.app.off('camera2:reset', this.resetToOverview, this);
};